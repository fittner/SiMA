 ...
 * 
 * @author wendt 25.02.2013, 14:40:42
 * 
 */
public class clsLongTermMemoryHandler implements itfModuleMemoryAccess
{
    ...
    public clsLongTermMemoryHandler(
            itfSearchSpaceAccess poSearchSpaceMethods
    )
    {
        moSearchSpaceMethods = poSearchSpaceMethods;
        moSpreadActivationHandler = new PsychicSpreadingActivation(moSearchSpaceMethods, moConsumeValue,
                mrActivationThreshold);
     ...
     * 
     * @see pa._v38.memorymgmt.itfModuleMemoryAccess#search(pa._v38.memorymgmt.enums.eDataType, java.util.ArrayList,
     * java.util.ArrayList)
     */
    @Override
    public <E> ArrayList<ArrayList<clsPair<Double, clsDataStructureContainer>>> searchEntity(
            eDataType poDataType, ArrayList<E> poPattern
    )
    {
        ArrayList<clsPair<Integer, clsDataStructurePA>> oSearchPattern = new ArrayList<clsPair<Integer, clsDataStructurePA>>();

        createSearchPattern(poDataType, poPattern, oSearchPattern); // Create a pattern, search for type, poDataType
                                                                    // 4096=TP, Input-Container
        ArrayList<ArrayList<clsPair<Double, clsDataStructureContainer>>> poSearchResult = moSearchSpaceMethods
                .searchEntity(oSearchPattern);
     ...
     * 
     * @see pa._v38.memorymgmt.itfModuleMemoryAccess#searchExactEntityFromInternalAttributes(java.lang.String,
     * java.lang.String, java.lang.String)
     */
    @Override
    public clsThingPresentationMesh searchExactEntityFromInternalAttributes(
            String poContent, String poShape, String poColor
            )
    {
        ...
        if (poShape != "")
        {
            clsThingPresentation oShapeTP = clsDataStructureGenerator
                    .generateTP(new clsPair<eContentType, Object>(eContentType.ShapeType, poShape));
        ...
        if (poColor != "")
        {
            clsThingPresentation oColorTP = clsDataStructureGenerator
                    .generateTP(new clsPair<eContentType, Object>(eContentType.COLOR, poColor));
            oPropertyList.add(oColorTP);
        }

        // Create the TPM
        clsThingPresentationMesh oGeneratedTPM = clsDataStructureGenerator
                .generateTPM(new clsTriple<eContentType, ArrayList<clsThingPresentation>, Object>(eContentType.ENTITY,
                        oPropertyList, poContent));
        ...
        for (int i = 0; i < oSearchResult2.size(); i++)
        {
            ...
            {
                oEmptySpaceContainerList.get(i).getMoAssociatedDataStructures()
                        .addAll(oSearchPair.get(0).b.getMoAssociatedDataStructures());
            }

        }

        ((clsThingPresentationMesh) oEmptySpaceContainer.getMoDataStructure())
                .setExternalAssociatedContent(oEmptySpaceContainer.getMoAssociatedDataStructures());
        clsThingPresentationMesh oEmptySpaceTPM = null;
        try
        {
            oEmptySpaceTPM = (clsThingPresentationMesh) ((clsThingPresentationMesh) oEmptySpaceContainer
                    .getMoDataStructure()).clone();
        }
     ...
     * @since 25.02.2013 14:59:22
     * 
     * @see pa._v38.memorymgmt.itfModuleMemoryAccess#executePsychicSpreadActivation(pa._v38.memorymgmt.datatypes.
     * clsThingPresentationMesh, double)
     */
    @Override
    public void executePsychicSpreadActivation(
            clsThingPresentationMesh primaryInput, ArrayList<clsDriveMesh> poDriveMeshFilterList,
            double prPsychicEnergyIn, int maxNumberOfDirectActivations, boolean useDirectActivation,
            double recognizedImageMultiplyFactor, ArrayList<clsThingPresentationMesh> secondaryInput
            )
    {
        ...
        moSpreadActivationHandler.startSpreadActivation(primaryInput, secondaryInput, recognizedImageMultiplyFactor,
                primaryInput, prPsychicEnergyIn, maxNumberOfDirectActivations, useDirectActivation,
                poDriveMeshFilterList, oAlreadyActivatedImages);
     ...
     */
    @Override
    public clsAssociationWordPresentation getSecondaryDataStructure(
            clsPrimaryDataStructure poDataStructure, double prThreshold
            )
    {
        ArrayList<ArrayList<clsPair<Double, clsDataStructureContainer>>> oSearchResult = new ArrayList<ArrayList<clsPair<Double, clsDataStructureContainer>>>();
        ...
        {
            oSearchResult = searchEntity(eDataType.WPM,
        ...
        {
            oSearchResult = searchEntity(eDataType.WP,
                    new ArrayList<clsDataStructurePA>(Arrays.asList(poDataStructure)));
        }
    ...
    public <E> void createSearchPattern(
            eDataType poDataType, ArrayList<E> poList, ArrayList<clsPair<Integer, clsDataStructurePA>> poSearchPattern
            )
    {
        ...
        for (E oEntry : poList)
        {
            ...
            {
                poSearchPattern.add(
                        new clsPair<Integer, clsDataStructurePA>(poDataType.nBinaryValue, (clsDataStructurePA) oEntry));
            }
            else if (oEntry instanceof clsPrimaryDataStructureContainer)
            {
                poSearchPattern.add(new clsPair<Integer, clsDataStructurePA>(poDataType.nBinaryValue,
                        ((clsPrimaryDataStructureContainer) oEntry).getMoDataStructure()));
            }
        }
    }
     ...
     * 
     * @see pa._v38.memorymgmt.itfModuleMemoryAccess#searchCompleteMesh(pa._v38.memorymgmt.datatypes.clsDataStructurePA,
     * int)
     */
    @Override
    public clsDataStructurePA searchCompleteMesh(
            clsDataStructurePA poInput, int pnLevel
            )
    {
        ...
        if (oRetVal == null)
        {
            try
            {
                throw new Exception("Error in searchMesh: the returned function for " + poInput
                        + " is null. This always occurs if images of TPM does not have any WPM");
            }
            catch (Exception e)
            {
                e.printStackTrace();
        ...
        // Move all associations from the found structure to the original structure of the input. This is used in
        // Spreadactivation where the mesh is "growing"
     ...
     * 
     * @see pa._v38.memorymgmt.itfModuleMemoryAccess#searchMesh(pa._v38.memorymgmt.clsDataStructurePA,
     * pa._v38.memorymgmt.eContentType, double, int)
     */
    @Override
    public ArrayList<clsPair<Double, clsDataStructurePA>> searchMesh(
            clsDataStructurePA poPattern, eContentType poSearchContentType, double prThreshold, int pnLevel
            )
    {
