...
public class Mav implements Steppable, Oriented2D {
	...
	public double[] nearbyMAVs(MavDemo mavdemo) {
		...
		final double d = mavdemo.sensorRangeDistance
				* mavdemo.sensorRangeDistance;

		final Bag nearbyMavs = mavdemo.mavs.getObjectsWithinDistance(
				new Double2D(x, y), 16, false, false);
		for (int i = 0; i < nearbyMavs.numObjs; i++) {
			final Mav mav = (Mav) (nearbyMavs.objs[i]);
			final double mavDistance = (mav.x - x) * (mav.x - x) + (mav.y - y)
					* (mav.y - y);
			if (mavDistance < d) // it's within our range
			{
				final int octant = sensorForPoint(mav.x, mav.y); // figure the
				// octant
				proximitySensors[octant] = /* Strict */Math.min(
						proximitySensors[octant], mavDistance);
			}
		}
	...
	public int currentSurface(MavDemo mavdemo) {
		for (int i = 0; i < mavdemo.region.length; i++)
			if (mavdemo.region[i].area.contains(x - mavdemo.region[i].originx,
					y - mavdemo.region[i].originy))
				return mavdemo.region[i].surface;
		return 0;
	}

	// in order to rotate 45/2 degrees counterclockwise around origin
	final double sinTheta = /* Strict */Math
			.sin(45.0 / 2 * /* Strict */Math.PI / 180);
	final double cosTheta = /* Strict */Math
			.cos(45.0 / 2 * /* Strict */Math.PI / 180);
