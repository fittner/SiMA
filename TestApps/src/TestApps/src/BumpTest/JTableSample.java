...
import EnergyInspector.JTreeSample;
...
public class JTableSample {
	...
	TestEnergyList testEnergyList=new TestEnergyList();
	private int botEnergy;
	private String botsensorId;
	...
	while(enum1.hasMoreElements()) //hasMoreElements()
	{
		...
		String sensorkey=node.toString();
		
		botEnergy=testEnergyList.getEnergyConsumptionTable(sensorkey);
		botsensorId=testEnergyList.SensorId(sensorkey);
		
//		temp.add(node);
//		temp.add(new Integer(node.getChildCount()));  //getMaxCap()
		temp.add(new String(botsensorId));  //getCurCap()
		temp.add(new Integer(botEnergy));
	...
	tree.addTreeWillExpandListener(new 
		...
		while(enum1.hasMoreElements())
		{
			...
			botEnergy=testEnergyList.getEnergyConsumptionTable(sensorkey);
			botsensorId=testEnergyList.SensorId(sensorkey);
			temp.add(new String(botsensorId));
			temp.add(new Integer(botEnergy));
