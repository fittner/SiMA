...
public class F57_MemoryTracesForDrives extends clsModuleBaseKB 
    ...
    public F57_MemoryTracesForDrives(String poPrefix, clsProperties poProp,
            ...
            itfModuleMemoryAccess poLongTermMemory, clsPersonalityParameterContainer poPersonalityParameterContainer, DT4_PleasureStorage poPleasureStorage, int pnUid) throws Exception {
            super(poPrefix, poProp, poModuleList, poInterfaceData, poLongTermMemory, pnUid);

        applyProperties(poPrefix, poProp); 
        
        moPleasure = poPleasureStorage.moPleasure;
...
//oDM.compareToDriveCandidate(poDriveCandidate)
                ...
                // Fittner: _Interface to STM/MTM/LTM
                oSearchResult = this.getLongTermMemory().searchEntity(eDataType.TPM, poSearchPattern);
                // Zwischenschichten einbauen STM / ITM / dann erst LTM 
                // Zuerst in KZG suchen, dann ITM dann LTM und calculation der Ergebnisse = Gesamtergebnis
                // Funktionen die ITM und LTM Daten übertragen
                ...
                ArrayList<clsPair<Double, clsDataStructureContainer>>  oSearchList2 = oSearchResult.get(0);
                
                if(F14_ExternalPerception.moSTM_DM != null)
                {
                    for ( clsDriveMesh Drive : F14_ExternalPerception.moSTM_DM)
                    {
                        ...
                        oSearchList2.add(new clsPair<Double, clsDataStructureContainer>(1.0, oDataStructureContainer));
                    }
                }
                
                //clsPair<Double, clsDataStructureContainer> oSTM_DM = oSearchList2.get((oSearchList2.size()-1));
                //clsDataStructurePA oSTM_DM2 = oSTM_DM.b.getMoDataStructure();
                //clsDriveMesh oSTM_DM3 = (clsDriveMesh) oSTM_DM2;
                //oSTM_DM3.setActualDriveObject(poDriveObject, prWeight);
                //oSTM_DM3.setActualDriveAim(poDriveAim, prWeight);
                ...
                for (ArrayList<clsPair<Double, clsDataStructureContainer>> oSearchList : oSearchResult){
                    ...
                    for (clsPair<Double, clsDataStructureContainer> oSearchPair: oSearchList) {
                        ...
                        //Merge STM and LTM
                        if(oMemoryDM.getActualDriveAim() != null && F14_ExternalPerception.moTPM_Action != null)
                        {
                            if(oMemoryDM.getActualDriveAim().isEquivalentDataStructure(F14_ExternalPerception.moTPM_Action) )
                            {
                                if(oMemoryDM.getActualDriveObject().isEquivalentDataStructure(F14_ExternalPerception.moTPM_Object) )
                                {
                                    oMemoryDM.setQuotaOfAffect(((oMemoryDM.getQuotaOfAffect()*0.5) + (moPleasure*(1-0.5))));
