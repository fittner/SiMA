 ...
 * 
 * @author wendt 27.09.2013, 21:26:28
 * 
 */
public class GoalHandlingFunctionality
{
     ...
     */
    public static ArrayList<clsWordPresentationMeshPossibleGoal> removeNonReachableGoals(
            ArrayList<clsWordPresentationMeshPossibleGoal> reachableGoalList, clsShortTermMemory shortTermMemory
    )
    {
     ...
     */
    public static void applyDriveDemandsOnReachableGoals(
            ArrayList<clsWordPresentationMeshPossibleGoal> selectableGoalList,
            ArrayList<clsWordPresentationMeshAimOfDrive> poDriveDemandList, double prDriveImpact
    )
    {
        GoalAlgorithmTools.applyDriveDemandsOnDriveGoal(selectableGoalList, poDriveDemandList, prDriveImpact);

        // ArrayList<clsWordPresentationMeshGoal> oSortedReachableGoalList =
        // clsGoalTools.sortAndEnhanceGoals(selectableGoalList, poDriveDemandList, currentFeelingsList,
        // affectThreashold);
    }
     ...
     */
    public static void applyFeelingsOnReachableGoals(
            ArrayList<clsWordPresentationMeshPossibleGoal> reachableGoalList,
            ArrayList<clsWordPresentationMeshFeeling> currentFeelings, double prFeelingsMatchImpact,
            boolean activateEmotionalInfluence, double receivedPsychicIntensity
    )
    {
        // HOW DO WE INCLUDE HERE THE receivedPsychicIntensity? How does it influence the choice of ordering the goals
        // by a given priority?
        // HOW DO WE REPORT THE QUANTITY OF PSYCHIC INTENSITY USED?

        // GoalGenerationTools.TEMP_METHOD_generatePanicGoal(reachableGoalList, currentFeelings,
        // activateEmotionalInfluence);

        moFeelingLog.debug("Step {}:", mnStep++);
        moFeelingLog.debug("Current feelings: ");
        /**
         * Every goal has a threshold. The thresholds are now hardcoded. It is so arranged that goalsByTriggeredFeeling
         * needs less psychic intensity and goalsByReservedFeeling more than goalsByTriggeredFeeling and
         * goalsByExpectedFeelingThreshold.
         */
        ...
        for (clsWordPresentationMeshPossibleGoal goal : reachableGoalList)
        {
            ...
            if (goal.getFeelings().isEmpty() == false)
            {
                ...
                if (receivedPsychicIntensity >= goalsByTriggeredFeelingThreshold
                        && receivedPsychicIntensity < goalsByExpectedFeelingThreshold)
                {
                    goal.setFeelingsMatchImportance(prFeelingsMatchImpact
                            * FeelingAlgorithmTools.evaluateGoalByTriggeredFeelings(goal, currentFeelings));
                }

                if (receivedPsychicIntensity >= goalsByExpectedFeelingThreshold
                        && receivedPsychicIntensity < goalsByReservedFeelingThreshold)
                {
                    goal.setFeelingsExpactationImportance(prFeelingsMatchImpact
                            * FeelingAlgorithmTools.evaluateGoalByExpectedFeelings(goal, currentFeelings));
                }

                if (receivedPsychicIntensity >= goalsByReservedFeelingThreshold)
                {
                    goal.setFeelingsMatchImportance(prFeelingsMatchImpact
                            * FeelingAlgorithmTools.evaluateGoalByReservedFeelings(goal, currentFeelings));
     ...
     * @param poRuleList
     */
    public static void applySocialRulesOnReachableGoals(
            ArrayList<clsWordPresentationMeshPossibleGoal> reachableGoalList, ArrayList<clsAct> poRuleList
            )
    {
        // TODO FG and/or SSch!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    }
    ...
    /**
     * The available reachable goals, may have aims (like EAT or DIVIDE) associated that might, or might not, fit the
     * aims originally spezified in the drives. for example: in primary process: a drive LIBIDINOUS/STOMACH might have
     * an object CAKE associated and an action EAT then the super ego forces the action to be replaced by DVIDE in
     * secondary process: two acts are extracted from memory that might be able to satisfy the drive: one about hitting
     * the cake, the other one about dividing it the act that contains information about dividing the cake should be
     * preferred, since it corresponds with the original drive aim (DIVID) from the primary process.
     * 
     * To achieve this, the function iterates over all reachable goals and, if the goal is associated to an act in
     * memory, tries to extract the action associated with that act.
     * 
     * If an action is found, it is compared to the action associated to the aim of drive it is supposed to satisfy
     * (e.g. LIBIDINOUSESTOMACH)
     * 
     ...
     *
     * @param poReachableGoals
     *            list of reachable goals (coming from perception or memory)
     * @param poAimOfDrives
     *            list of relevant drive aims
     */
    public static void applyAimImportanceOnReachableGoals(
            ArrayList<clsWordPresentationMeshPossibleGoal> poReachableGoals,
            ArrayList<clsWordPresentationMeshAimOfDrive> poAimOfDrives, double prGoalImpact
            )
    {
        double nImportance;
        String oGoalName;
    ...
    /**
     * DOCUMENT - Compares a possible goal to the current perception to determine how well the attributes of entities in
     * the acts intention act match actual entities
     * 
     * Currently this comparison consists of: 1) extract intention 2) extract all entities in intention 3) try to find
     * matching entities in perception 4) calculate match values for all entities (if the entity is not present in the
     * perception it will NOT be considered - determining if the required entities are present at all should be handled
     * by another method reason for this decission is: focusing on entities requires an internal action, it is possible
     * to have many steps that are not yet focused and in this cases acts with more attributes associated to the
     * entities would get unnaturally strong negative importance) 5) combine all match values into a single importance
     * value
     *
     * @author kollmann
     ...
     *
     * @param poReachableGoals
     *            List of all reachable goals (should already be filtered by available NI)
     * @param poFocusedPerception
     *            WPM Image containing the information of all perceived entities that are currently focused.
     */
    public static void applyObjectAttributeMatchImportanceOnPossibleGoals(
            List<clsWordPresentationMeshPossibleGoal> poReachableGoals, clsWordPresentationMesh poFocusedPerception
            )
    {
        ...
        for (clsAssociationSecondary oEntityAssociation : clsAssociationSecondary
                .filterListByPredicate(poFocusedPerception.getInternalAssociatedContent(), ePredicate.HASPART))
        {
            oRawDS = oEntityAssociation.getTheOtherElement(poFocusedPerception);

            if (oRawDS instanceof clsWordPresentationMesh
                    && !((clsWordPresentationMesh) oRawDS).getContent().equals("SELF"))
            {
        ...
        for (clsWordPresentationMeshPossibleGoal oGoal : poReachableGoals)
        {
            ...
            if (oSupportiveDataStructure.getContentType().equals(eContentType.ACT))
            {
                ...
                for (clsAssociationSecondary oEntityAssociation : clsAssociationSecondary
                        .filterListByPredicate(oIntention.getInternalAssociatedContent(), ePredicate.HASPART))
                {
                    ...
                    if (oRawDS instanceof clsWordPresentationMesh)
                    {
                        ...
                        for (clsWordPresentationMesh oPerceivedEntity : oPerceivedEntities)
                        {
                            ...
                            {
                                oPairs.add(new clsPair<clsWordPresentationMesh, clsWordPresentationMesh>(
                                        oPerceivedEntity, oRIEntity));
                            }
                        }
                    }
                    else
                    {
                        log.error(
                                "Intention has a HASPART association that does not point to a WPM:\nIntention: {}\nAssociation:{}",
                                oIntention, oRawDS);
                    }
                }
                ...
                for (clsPair<clsWordPresentationMesh, clsWordPresentationMesh> oPair : oPairs)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            oBodystateAssociations = clsAssociationSecondary.filterListByPredicate(
                                    oPair.a.getExternalAssociatedContent(), ePredicate.HASBODYSTATE);
                            if (oBodystateAssociations.size() > 0)
                            {
                                if (oBodystateAssociations.size() > 1)
                                {
                                    log.warn(
                                            "Entity {} has more than one bodystate associated - will be usind the first one",
                                            oPair.a);
                                }
                            ...
                            oBodystateAssociations = clsAssociationSecondary.filterListByPredicate(
                                    oPair.b.getExternalAssociatedContent(), ePredicate.HASBODYSTATE);
                            if (oBodystateAssociations.size() > 0)
                            {
                                if (oBodystateAssociations.size() > 1)
                                {
                ...
                if (nNumValidMatchings > 0)
                {
                    ...
                    oGoal.setEntityValuationImportance((rValuationMatch / nNumValidMatchings)
                            * oGoal.getDriveDemandImportance() * mrEntityValuationMatchImpact);
                    oGoal.setEntityBodystateImportance((rAttributedMatch / nNumValidMatchings)
                            * oGoal.getDriveDemandImportance() * mrEntityBodystateMatchImpact);
                }
            }
        }
    }
     ...
     * @param reachableGoalList
     */
    public static ArrayList<clsWordPresentationMeshPossibleGoal> selectSuitableReachableGoals(
            ArrayList<clsWordPresentationMeshPossibleGoal> reachableGoalList, int numberOfGoalsToPass
            )
    {
        ArrayList<clsWordPresentationMeshPossibleGoal> result = clsGoalManipulationTools
                .sortAndFilterGoalsByTotalImportance(reachableGoalList, numberOfGoalsToPass);
     ...
     * @param reachableGoalList
     */
    public static clsWordPresentationMeshPossibleGoal selectPlanGoal(
            ArrayList<clsWordPresentationMeshPossibleGoal> reachableGoalList
            )
    {
        ArrayList<clsWordPresentationMeshPossibleGoal> resultList = clsGoalManipulationTools
                .sortAndFilterGoalsByTotalImportance(reachableGoalList, 1);
     ...
     * @return
     */
    public static ArrayList<clsWordPresentationMeshAimOfDrive> sortAimOfDrives(
            ArrayList<clsWordPresentationMeshAimOfDrive> aimOfDrivesList
            )
    {
        ArrayList<clsWordPresentationMeshAimOfDrive> result = clsGoalManipulationTools
                .sortAndFilterGoalsByTotalImportance(aimOfDrivesList, -1);
     ...
     * 
     * 
     * Extract goals from the perception, which are emotions and are high negative in order to be able to react faster.
     * 
     * (wendt)
     *
     ...
     * @return
     */
    private static ArrayList<clsPair<Double, clsWordPresentationMeshGoal>> extractStrongestPerceptiveGoals(
            ArrayList<clsWordPresentationMeshGoal> poReachableGoalList, double strongestGoalThreshold
            )
    {
        ...
        for (clsWordPresentationMeshGoal oReachableGoal : poReachableGoalList)
        {
            ...
            {
                oRetVal.add(new clsPair<Double, clsWordPresentationMeshGoal>(oReachableGoal.getTotalImportance(),
                        oReachableGoal));
            }
        }
     ...
     * @return
     */
    public static ArrayList<clsWordPresentationMeshPossibleGoal> extractSelectableGoalsFromActs(
            ArrayList<clsWordPresentationMesh> moActList
            )
    {
     ...
     * @return
     */
    public static ArrayList<clsWordPresentationMeshPossibleGoal> extractSelectableGoalsFromPerception(
     ...
     * @return
     */
    public static ArrayList<clsWordPresentationMeshPossibleGoal> extractSelectableGoalsFromAimOfDrives(
            ArrayList<clsWordPresentationMeshAimOfDrive> aimOfDrives
            )
    {
     ...
     * @return
     */
    private ArrayList<clsWordPresentationMeshGoal> extractEmergentGoalsFromEmotions(
            ArrayList<clsWordPresentationMeshGoal> poGoalList
            )
    {
    ...
    public static ArrayList<clsWordPresentationMeshGoal> extractGoalFromContext(
            ArrayList<clsWordPresentationMeshPossibleGoal> moReachableGoalList_OUT
            )
    {
        ArrayList<clsWordPresentationMeshGoal> oRetVal = new ArrayList<clsWordPresentationMeshGoal>();
