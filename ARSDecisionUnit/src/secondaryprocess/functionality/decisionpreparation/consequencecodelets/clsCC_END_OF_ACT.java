 ...
 * 
 * @author wendt 27.09.2012, 14:51:02
 * 
 */
    ...
    )
    {
        ...
        if (oTPM != null && !oTPM.isNullObject())
        {
            for (clsAssociation oAssoc : oTPM.getExternalAssociatedContent())
            {
                ...
                {
                    clsThingPresentationMesh oDriveAim = ((clsDriveMesh) oAssoc.getAssociationElementA())
                            .getActualDriveAim();
        ...
        for (clsDriveMesh oDriveMesh : oDrives)
        {
            ...
            {
                buffer.receive_D1_3(oDrive, new clsPair<Double, Double>(oDriveMesh.getPsychicSatisfactionValue(), 0.0));
            }
            else
            {
                buffer.receive_D1_3(oDrive, new clsPair<Double, Double>(0.0, oDriveMesh.getPsychicSatisfactionValue()));
            }
        }
