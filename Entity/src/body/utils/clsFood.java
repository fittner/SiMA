...
public class clsFood {
	 ...
	 *
	 * @param prWeight
	 * @throws utils.exceptions.exFoodWeightBelowZero 
	 */
	public void setWeight(double prWeight) throws utils.exceptions.exFoodWeightBelowZero {
		mrWeight = prWeight;
		
		if (mrWeight < 0.0f) {
			mrWeight = 0.0f;
			
			throw new utils.exceptions.exFoodWeightBelowZero();
		}
	}
	 ...
	 */
	public double getNutritionWeight(eNutritions poNutritionId) throws utils.exceptions.exFoodNotFinalized {
		if (!mnFinalized) {
			throw new utils.exceptions.exFoodNotFinalized();
		}
...
//		return mrWeight * moComposition.get(poNutritionId).floatValue();
	 ...
	 * @throws utils.exceptions.exFoodNotFinalized
	 */
	public HashMap<eNutritions, clsMutableDouble> getNutritionWeights() throws utils.exceptions.exFoodNotFinalized {
		if (!mnFinalized) {
			throw new utils.exceptions.exFoodNotFinalized();
		}
	 ...
	 * @param poFraction
	 */
	public void addNutritionFraction(eNutritions poNutritionId, clsMutableDouble poFraction) throws utils.exceptions.exFoodAlreadyNormalized {
		if (mnFinalized) {
			throw new utils.exceptions.exFoodAlreadyNormalized();
		}
	 ...
	 *
	 */
	private void normalizeFractions() throws utils.exceptions.exFoodAlreadyNormalized {
		if (mnFinalized) {
			throw new utils.exceptions.exFoodAlreadyNormalized();
		}
	...
	@Override
	public void finalize() throws utils.exceptions.exFoodAlreadyNormalized {
		normalizeFractions();
		mnFinalized = true;
	}
