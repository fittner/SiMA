...
public class clsBodyOrganArms extends clsBodyOrgan{
	...
	public void affectMuscleTension(ArrayList<Double> poStorageOfEmotionIntensities, ArrayList<String> poStorageOfEmotionNames)	{
		...
		for(int a = 0; a < poStorageOfEmotionIntensities.size(); a++){
			if ( poStorageOfEmotionNames.get(a).equalsIgnoreCase( "ANGER" ) ){
				...
				addTensionIntensity( poStorageOfEmotionIntensities.get(a) * (+1) * ( this.getAngerFactor() ) ); // (+) plus, adds tension
			}
				...
				addTensionIntensity( poStorageOfEmotionIntensities.get(a) * (+1) * ( this.getAnxietyFactor() ) ); // (+) plus, adds tension
			}
				...
				addTensionIntensity( poStorageOfEmotionIntensities.get(a) * (-1) * (this.getElationFactor()) ); // (-) minus, relieves tension
			} // no mucle tension caused by other emotions at the moment..
	/*		else
			{ // for other emotions..
