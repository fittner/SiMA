    ...
    private boolean checkForEmotions(int emotionSize, int position) {
        ...
        for (e = 0, beGefunden = true; e < emotionSize && beGefunden; e++) {
            ...
            for(clsEmotion oOneEmotion : moEmotions_Input) {
                ...
                if (oOneEmotion.getMoContent().equals(oRegeln.get(position).getEmotionRule(e).a) &&
        ...
        for (p = 0, bPerception = true; p < perceptionSize && bPerception; p++) {
            ...
            for (clsAssociation oAssociation : oInternalAssociations) {
                if (oAssociation.getMoAssociationElementB() instanceof clsThingPresentationMesh) {
                    if( ((clsThingPresentationMesh)oAssociation.getMoAssociationElementB()).getMoContentType().equals(oRegeln.get(position).getPerceptionRule(p).a) &&
                            ((clsThingPresentationMesh)oAssociation.getMoAssociationElementB()).getMoContent().equals(oRegeln.get(position).getPerceptionRule(p).b) ) {
