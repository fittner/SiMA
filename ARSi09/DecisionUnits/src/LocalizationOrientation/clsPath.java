...
public class clsPath {
	...
	/**
	 * 
	 * @return the first step within the list
	 */
	...
	/**
	 * 
	 * @return the next step in the list. 
	 */
	...
	public double getPathQuality(clsAreaSemanticMemory SemAreaMemory){
		...
		for (int i=0; i < this.Steps.numObjs;i++)
			quality += 100-SemAreaMemory.getAreaEntry(((clsStep)this.Steps.get(i)).getCurrentArea() -1 ).getUpToDateness() + 1; //the +1 i done so that even if all areas are up to date, the amout of the areas is counted and used for finding the shortest/best
	...
	/**
	 * resets the path. no transition was yet reached
	 * @return 
	 */
	...
	//prints the areas the path crosses
	public void String(){
		int i;
